...
Use this skill when work requires deterministic local validation and handoff readiness.
...
- Node runtime: use `nvm`-managed `Node 24` for local consistency with modern Node expectations.
...
3. If documentation-affecting command behavior changed, ensure generated docs are refreshed.
4. Use `conventional commits` when a commit is requested.

## Boundaries
This skill does not define architecture, command class selection, or rendering strategy. Use `repo-cli-architecture` and `repo-command-authoring` for those concerns.
