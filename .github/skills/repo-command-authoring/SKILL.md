...
`ExecRenderBaseCommand` does not provide real-time progress output by itself.
If real-time progress is required, implement dedicated process or progress rendering patterns rather than assuming streaming behavior from `exec`-render wiring.

## Context-Aware Command Pattern
1. Determine whether project or organization scope is required.
2. Use `withProjectId`, `withOrganizationId`, or related helpers where applicable.
3. Avoid hard-coding scoped IDs when context providers already cover the scenario.
...
## Boundaries
This skill focuses on command implementation quality. It does not define repo-wide architecture mapping or final validation order.
