...
## Architectural Map
- Command entrypoints: `src/commands` by domain (`app`, `backup`, `container`, and others)
...
Use context helpers such as `withProjectId` and `withOrganizationId` in context-aware commands.
...
## Boundaries
This skill does not define validation command order or release hygiene. Use `repo-development-workflow` for that.
